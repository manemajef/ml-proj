...
def _(mo):
    ...
    - The corr between the Missing Agent and has client id makes sense, since they provide alternative ways to assign to the course

    - `Prev_Course_Attended <--> Returning_Client: 0.44` seem to not make sense. If a person has attended prev course, that must imply hes a returning client no ? I would treat it as an error in the data, and choose to drop one of them.
    - weve seen already theres a strong corr between early tregistration and drops.
    - we see that out of all the returning, there is a similar amount of drops, that implies that a lot of dropouts are just due to dicculty, and people may return and try again.
      - It would be intresting to see what is the drop rate of `prev_course && prev_drop` in curr.
    """)
    return
...
def _(mo):
    mo.md(r"""
    ### Conclusions from Corr with drops
    """)
    return
